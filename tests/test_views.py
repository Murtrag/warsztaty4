...
@pytest.mark.parametrize(
	...
	[
		...
		((datetime.datetime.now() - datetime.timedelta(1)).strftime("%Y-%m-%d"), 302, 'przeszłości'), #yesterday
		((datetime.datetime.now() + datetime.timedelta(1)).strftime("%Y-%m-%d"), 302, 'zarezerwowana'), #tomorrow booked

	]
	)
@pytest.mark.views
def test_view_reseration(client, dummy_room,  data, result_status, message):
	dummy_reservation = mixer.blend(
		'page.Reservation',
